# all the imports
import os
from flaskext.mysql import MySQL
from flask import Flask, request, session, g, redirect, url_for, abort, \
    render_template, flash
from flask_memcache_session import Session
from werkzeug.contrib.cache import MemcachedCache

app = Flask(__name__)  # create the application instance :)

# Memcached Config
memcached_host = os.getenv('MEMCACHED_HOST', 'memcached')
memcached_port = os.getenv('MEMCACHED_PORT', '11211')
app.cache = MemcachedCache([memcached_host, memcached_port])
app.session_interface = Session()

app.config.from_object(__name__)  # load config from this file , flaskr.py

# Database config
mysql = MySQL()
app.config['MYSQL_DATABASE_HOST'] = os.getenv('DATABASE_HOST', 'mysql')
app.config['MYSQL_DATABASE_USER'] = os.getenv('DATABASE_USER', 'root')
app.config['MYSQL_DATABASE_PASSWORD'] = os.getenv('DATABASE_PASS')
app.config['MYSQL_DATABASE_DB'] = os.getenv('DATABASE_DB')
mysql.init_app(app)

# Load default config and override config from an environment variable
app.config.update(dict(
    SECRET_KEY='development key',
    USERNAME='admin',
    PASSWORD='default'
))
app.config.from_envvar('FLASKR_SETTINGS', silent=True)


def connect_db():
    """Connects to the specific database."""
    rv = mysql.connect()
    return rv


def init_db():
    db = get_db()
    with app.open_resource('schema.sql', mode='r') as f:
        for line in f:
            db.cursor().execute(line)
    db.commit()


@app.cli.command('initdb')
def initdb_command():
    """Initializes the database."""
    init_db()
    print('Initialized the database.')


def get_db():
    """Opens a new database connection if there is none yet for the
    current application context.
    """
    if not hasattr(g, 'db_conn'):
        g.db_conn = connect_db()
    return g.db_conn


@app.teardown_appcontext
def close_db(error):
    """Closes the database again at the end of the request."""
    if hasattr(g, 'db_conn'):
        g.db_conn.close()


@app.route('/')
def show_entries():
    db = get_db()
    cur = db.cursor()
    cur.execute('select title, text from entries order by id desc')
    entries = cur.fetchall()
    return render_template('show_entries.html', entries=entries)


@app.route('/add', methods=['POST'])
def add_entry():
    if not session.get('logged_in'):
        abort(401)
    query = 'insert into entries (title, text) \
        values (\'{0}\', \'{1}\')'.format(
        request.form['title'], request.form['text'])
    db = get_db()
    db.cursor().execute(query)
    db.commit()
    flash('New entry was successfully posted')
    return redirect(url_for('show_entries'))


@app.route('/login', methods=['GET', 'POST'])
def login():
    error = None
    if request.method == 'POST':
        if request.form['username'] != app.config['USERNAME']:
            error = 'Invalid username'
        elif request.form['password'] != app.config['PASSWORD']:
            error = 'Invalid password'
        else:
            session['logged_in'] = True
            flash('You were logged in')
            return redirect(url_for('show_entries'))
    return render_template('login.html', error=error)


@app.route('/logout')
def logout():
    session.pop('logged_in', None)
    flash('You were logged out')
    return redirect(url_for('show_entries'))
